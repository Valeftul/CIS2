...
export interface Product {
  ...
  quantity: number;
  unit: string;
  purchasePrice: number; // внутренняя цена — не передаётся клиентам
  minQuantity: number;
  supplier: string;
  description: string;
  updatedAt: string;
...
export interface Contract {
  ...
  // Товар (без закупочной цены — коммерческая тайна клиента)
  productName: string;
  productSku: string;
  quantity: number;
  unit: string;
  ...
  // Страхование товара
  insuranceEnabled: boolean;       // выбрал ли клиент страхование
  declaredValue: number;           // объявленная клиентом стоимость товара (₽)
  insurancePremium: number;        // страховой взнос = declaredValue * 10%
  insuranceCompany: string;        // название страховой компании
